...
This repository manages the declarative NixOS and Home Manager configurations for `server` and `laptop` hosts.
...
# Build server host dryly (evaluates configuration without switching)
...
- **Host-Specific User Tools**: Place in `hosts/<host>/home.nix` (e.g., Niri utilities on laptop, diagnostic tools on server).
...
### 5. Host Isolation & Homelab Infrastructure
- Keep machine-specific homelab infrastructure (`homeserver.nix`, `traefik-deployments.nix`, `dynu.nix`) under `hosts/server/`. Never import them into `hosts/laptop/`.
- Protect the headless `server` profile. Any changes to shared base modules must preserve the headless server runtime.
...
| **MEDIUM** | Shared System & Workstation | `modules/system/*`, greetd/display manager, firewall rules, user groups, new flake inputs. | **Autonomous**: Agent plans, edits, and validates via `nix flake check` + builds **both** `server` and `laptop`. |
| **HIGH** | Core & Infrastructure | Bootloader, filesystems, SOPS keys/secrets setup, server deployments, Traefik, Docker networking. | **Stop & Wait**: Agent must present the plan and **wait for user approval before editing**. |
...
| :--- | :--- | :--- |
| `modules/system/*` | `server`, `laptop` | `nix flake check`<br>`nix build .#nixosConfigurations.server.config.system.build.toplevel --no-link`<br>`nix build .#nixosConfigurations.laptop.config.system.build.toplevel --no-link` |
| `modules/user/*` or `home.nix` | `server`, `laptop` | `nix flake check`<br>`nix build .#nixosConfigurations.server.config.system.build.toplevel --no-link`<br>`nix build .#nixosConfigurations.laptop.config.system.build.toplevel --no-link` |
| `hosts/server/*` | `server` only | `nix flake check`<br>`nix build .#nixosConfigurations.server.config.system.build.toplevel --no-link` |
| `hosts/laptop/*` | `laptop` only | `nix flake check`<br>`nix build .#nixosConfigurations.laptop.config.system.build.toplevel --no-link` |
| `flake.nix` / `flake.lock` | `server`, `laptop` | `nix flake check` + build both hosts |
